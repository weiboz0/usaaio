...
def write_repo(root: Path, problems: list[dict], status: str | None = "final") -> None:
    ...
    (BOOK1_ROOT / "mocktests/blueprint.yaml").resolve().read_text()
    root.joinpath("mocktests/blueprint.yaml").write_text((BOOK1_ROOT / "mocktests/blueprint.yaml").read_text())
    root.joinpath("syllabus.md").write_text((BOOK1_ROOT / "syllabus.md").read_text())
...
def test_blueprint_vacuous_without_mocktests(tmp_path):
    ...
    (tmp_path / "mocktests" / "blueprint.yaml").write_text((BOOK1_ROOT / "mocktests/blueprint.yaml").read_text())
    (tmp_path / "syllabus.md").write_text((BOOK1_ROOT / "syllabus.md").read_text())
    assert check_blueprint(tmp_path).ok
